...
class Dataset(object):
    ...
    rawdata_map = {
        'motion': motion_type,
        'sound': sound_level2_type,
        'location': location_level2_type
    }
    ...
    def __init__(self, obs=None):
        if obs is not None:
            # self.seq = seq
            self.obs = obs
            self.dataset = self._convertObs2Dataset(obs)
    ...
    def _convertNumericalObservation(self, obs):
        ...
        for seq in obs:
            spots_set = []
            for senz in seq:
                # TODO: Read rawdata type from rawdata_map in a 'for'
                spot = [
    ...
    # TODO: input is numerical matrix, and convert it to dataset format (It will invoke _convertNumericalObservation first)
    ...
    def _generateRawdataRandomly(self, rawdata_type, results_prob_list):
        ...
        for result_prob in results_prob_list:
            ...
            if result_prob.keys()[0] not in self.rawdata_map[rawdata_type] and result_prob.keys()[0] is not 'Others':
                return None
